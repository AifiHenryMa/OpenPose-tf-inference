...
## 2.3 需要测试的视频
video_dir = './videos/'
video_name = "fat_people_fall_down.avi"
...
######------------------------------------------------------------------#######
# 5. 定义MyTest_image函数
def MyTest_image(testimage=image, testmodel=model, outputimage='result2.jpg', upsamp_size=[1312, 736]):
...
######------------------------------------------------------------------#######
def MyTest_video(input_video=video, testmodel=model, outputimage='result3.avi', upsamp_size=[1312, 736]):
	# 加载pb模型
	with tf.gfile.GFile(testmodel, 'rb') as f:
		graph_def = tf.GraphDef()
		graph_def.ParseFromString(f.read())
	...
	with tf.Session(graph=graph_OpenPose) as sess:
		...
		cap = cv2.VideoCapture(input_video) # CV读入视频文件
		frame_flag = 0
		
		fourcc = cv2.VideoWriter_fourcc(*'XVID')
		videoWriter = cv2.VideoWriter(outputimage, fourcc, 20.0, (1280, 720))
		...
		while cap.isOpened():
			...
			if ret_val == 1:
				(peaks, heatMat_up, pafMat_up) = sess.run(
												 	  	  [tensor_peaks, tensor_heatMat_up, tensor_pafMat_up],
												 	  	  feed_dict={tensor_image:[frame], upsample_size:upsamp_size}
									             	 	 )
				peaks = peaks[0]
				heatMat = heatMat_up[0]
				pafMat = pafMat_up[0]
				humans = estimate_paf(peaks, heatMat, pafMat)
				frame_output = draw_humans(frame, humans, imgcopy=False)
				frame_flag = frame_flag + 1
				print (frame_flag) # 输出是第多少帧
				print (frame_output)
